...
public class MenuManager implements Listener
{
        ...
        if (event.getCurrentItem() != null && event.getCurrentItem().getType() == Material.NETHER_STAR && event.getCurrentItem().getItemMeta().getDisplayName() != null && event.getCurrentItem().getItemMeta().getDisplayName().equals(String.valueOf(ChatColor.GOLD.toString()) + ChatColor.BOLD + "MENU")) {
            try {
                ...
                if (event.getCurrentItem().getItemMeta().getLore().equals(loresList)) {
                    ...
                    Bukkit.getScheduler().runTaskLater(Main.INSTANCE, () -> { GuiManager.toOpen(player, PlayerProfileManager.profiles.get(player).getHashMapInventoryTypeToInventory().get(GuiManager.InventoryTypeList.INVENTORYPLAYERMENU));
                    }, 10);
    ...
    public static void initializePlayerMenuItems(final Player player, final Inventory inventory) {
        inventory.setItem(4, GuiItem.createHeadItem(Material.PLAYER_HEAD, player, ChatColor.GREEN + "Profil","", ChatColor.GRAY + "Rang " + PlayerProfileManager.profiles.get(player).getRank().getColor() + PlayerProfileManager.profiles.get(player).getRank().getDisplayName(), ChatColor.GRAY + "Classe " + PlayerProfileManager.profiles.get(player).getClasses().getColor() + PlayerProfileManager.profiles.get(player).getClasses().getDisplayName(),"" , ChatColor.GRAY + "Niveau " +(int) PlayerProfile.getStat(player, PlayerProfile.Stat.LEVEL), "", ChatColor.RED.toString() + "\u2764 Vitalit\u00e9 " + ChatColor.GRAY + (int) PlayerProfile.getStat(player, PlayerProfile.Stat.VITALITYNATIVE), ChatColor.BLUE.toString() + "\u2748 Defence " + ChatColor.GRAY + (int) PlayerProfile.getStat(player, PlayerProfile.Stat.DEFENCENATIVE), ChatColor.GREEN.toString() + "\u03a8 Force " + ChatColor.GRAY + (int) PlayerProfile.getStat(player, PlayerProfile.Stat.STRENGTHNATIVE), ChatColor.YELLOW.toString() + "\u27b6 Dexterit\u00e9 " + ChatColor.GRAY + (int) PlayerProfile.getStat(player, PlayerProfile.Stat.DEXTIRITYNATIVE), ChatColor.BLUE.toString() + "\u2745 Intelligence " + ChatColor.GRAY + (int) PlayerProfile.getStat(player, PlayerProfile.Stat.INTELLIGENCENATIVE), ChatColor.DARK_PURPLE.toString() + "\u29fe Foi " + ChatColor.GRAY + (int) PlayerProfile.getStat(player, PlayerProfile.Stat.FAITHNATIVE), String.valueOf(ChatColor.WHITE.toString()) + "\u2727 Agilit\u00e9 " + ChatColor.GRAY + (int) PlayerProfile.getStat(player, PlayerProfile.Stat.AGILITYNATIVE), "", String.valueOf(ChatColor.LIGHT_PURPLE.toString()) + "\u2756 Karma " + ChatColor.GRAY + (int) PlayerProfile.getStat(player, PlayerProfile.Stat.KARMA)));
    ...
    public static void initializePlayerMenuDevItems(final Player player, final Inventory inventory) {
        ...
        inventory.setItem(32, GuiItem.createGuiItem(new ItemStack(Material.COMPARATOR), ChatColor.RED + "Paramètres", ""));
        if (player.isOp()) {
            inventory.setItem(30, GuiItem.createGuiItem(new ItemStack(Material.GRASS_BLOCK), ChatColor.GREEN + "NormalMod", ""));
        }
    }
}
